...
export const changePassword = async (
  ...
  try {
    ...
    const user = await User.findById(req.user.userId).select(
      "+authentication.password +authentication.salt +authentication.sessionId"
    );
    if (!user) {
      res.status(404).json({
    ...
    await redisClient.set(`bl:${user.authentication.sessionId}`, "true", {
      EX: 60 * 15,
    }); // Blacklist old accessToken if needed
